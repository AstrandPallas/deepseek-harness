...
| `retries` | 对失败的前台委派在终止原因为可重试（`error` 或 `max-tokens`）时的自动重跑次数，默认 `0`。每次尝试在启动下一次前都会 dispose 自己的运行；达到上限后的出错结果仍会附带最后一次尝试的部分文本。后台运行绝不自动重试。 |
...
当提供方存在时，以当前实例配置的名称公开已生成的默认 [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent)。提供方是否继承上下文会改变工具描述和提示词描述。启用后台模式会添加 `run_in_background`：可继续模式会记录其默认值为 `true`、运行时结算通知与显式前台覆盖；一次性模式会记录其默认值为 `false`，以及用 `job_output` 收集或用 `job_kill` 停止的 job id。具备 `outputSchema` 能力的提供方会添加 `output_schema`，一个以 object 为根的 JSON Schema，用于约束子 agent 的最终结构化结果；针对无能力提供方绕过 schema 夹带的使用会在执行时被拒绝。当工具在本次组装的作用域中可见时，一个 `tool:<toolName>` 系统提示词 section 会指示模型同时启动相互独立的可继续委派、在它们运行时继续工作，并且仅当下一步动作依赖结果时选择前台；工具限制会同时移除其 schema 和这段指引。
...
调用会保留描述和提示词。成功时只包含子 agent 的最终文本，或者，当调用请求了 `output_schema` 且子 agent 捕获到结构化结果时，包含子 agent 报告的已校验 JSON 对象。其他结果变为 `Error: <message>`。子 agent 中间步骤不会进入父级。
