...
受支持的 profile 字段是 `apiKeyEnv`、`displayName`、`api`、`baseURL`、`models`、`modelOverrides`、`compat`、`defaultContextWindow`、`defaultMaxTokens`、`defaultInput`、`headers`、`reasoning`、`thinkingBudgets`、`cacheRetention`、`transport`、`timeoutMs`、`websocketConnectTimeoutMs`、`streamIdleTimeoutMs`、`maxConcurrent`、`kvUnits`、`kvPool` 和 `retryPolicy`。`maxConcurrent` 上限让超出限制的流按到达顺序排队（按本地服务器的 slot 数设置，例如 llama.cpp 的 `-np`）；省略则路由不受限。`kvUnits` 与 `kvPool` 为共享同一本地服务器的路由声明加权预算：每个请求在容量为 `kvPool` 的共享池中预留 `kvUnits`，以 `baseURL` 为键覆盖所有声明该预算的路由；大请求会等待小请求耗尽，而不会阻塞新到的小请求。这两个字段必须同时设置、配合显式 `baseURL`，且 `kvUnits` 不得超过 `kvPool`。每个 profile 的可选重试策略都会与该提供方路由一同捕获；省略时使用有界的常规默认值。流空闲间隔必须是正的有限 Node 定时器延迟，默认为五分钟，且只覆盖未完成提供方读取，不包括消费方思考时间。若已配置标头中有同名项，则以 Harness 应用归因为准。
