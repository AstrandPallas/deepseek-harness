...
export const Config: z<Config> = z.object({
  stopCommand: z.string().default("Set-Content 'C:/LocalModel/qwen/watchdog.stop' 'stop'; taskkill /IM ninfer-serve.exe /F 2>&1 | Out-Null; if (Test-Path 'C:/LocalModel/qwen/watchdog.pid') { $wd = Get-Content 'C:/LocalModel/qwen/watchdog.pid'; Stop-Process -Id ([int]$wd) -Force -ErrorAction SilentlyContinue }; Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList @('-NoProfile','-Command','nvidia-smi -pl 575') | Out-Null; Write-Output 'local model stopped'"),
  startCommand: z.string().default("Remove-Item 'C:/LocalModel/qwen/watchdog.stop' -ErrorAction SilentlyContinue; Start-Process powershell -ArgumentList @('-ExecutionPolicy','Bypass','-WindowStyle','Hidden','-File','G:/LocalModel/watchdog-llama.ps1'); Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList @('-NoProfile','-Command','nvidia-smi -pl 530') | Out-Null; Write-Output 'local model watchdog started'"),
