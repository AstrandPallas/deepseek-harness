...
For `pressure`, compaction-basic resolves the durable provider/model target's adapter-owned capacity and exact-target policy, then applies the resulting threshold and retained-tail budgets to one unified `ctx.tokenMeter.measure()` result. The threshold compares the prompt plus the reserved completion — the durable request header's effective `config.maxTokens`, or the owning adapter's `defaultMaxTokens` before the first routed request — because providers reject a request whose prompt plus `max_tokens` exceeds the window; the [reservation-aware pressure note](../feature/2026-08-14-compaction-pressure-counts-reservation.md) owns that adjustment. Below pressure it returns without pruning. Once pressure qualifies, optional `ctx.toolResultPruner` rewrites oversized current results and compaction-basic remeasures through the same meter; safe pressure skips the model call, while remaining pressure selects and summarizes from the pruned surface. The same singleton meter owns range pricing, cited source-event accounting, shadowed token counts, and non-shrinking-summary rejection. Common defaults remain threshold ratio `0.8`, retained-history ratio `0.16`, summarization provider/model `''`, `maxTokens: 8192`, `compactionRetries: 1`, and `auto: true`; optional `modelPolicies` entries override them for an exact provider/model pair.
